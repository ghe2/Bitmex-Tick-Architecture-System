var biPartite = function (colors, height, bb, decimalPlaces, c1, c2, c3, partWidth, min,displayLabel,displayValue,displayPercent,smartNumberFormatter) {
    var bP = {};
    var b = min;
    var buffMargin = 1;
    var minHeight = min;
        ...
        function calculatePosition(a, s, e, b, m) {
            ...
            a.forEach(
				function (d) {
				    ...
				    v.value = d;
				    v.height = v.percent * (e - s - 2 * b * a.length);
            ...
            ret.forEach(
				function (d) {
				    ...
				    d.middle = sum + b + d.height ;
				    d.y = s + d.middle - d.percent * (e - s - 2 * b * a.length);
    ...
    function drawPart(data, id, p) {
        ...
        mainbar.append("text").attr("class", "barlabel")
			.attr("x", c1[p]).attr("y", function (d) { return d.middle + 5; })
						.text(function (d, i) {
                return data.mainBars[p][i].height > 5 ? data.keys[p][i] :"";
            })
			.attr("text-anchor", "start")
            .attr("display", displayLabel);

        mainbar.append("text").attr("class", "barvalue")
			.attr("x", c2[p]).attr("y", function (d) { return d.middle + 5; })
			.text(function (d, i) { 
                return smartNumberFormatter ? smartNumberFormatter(d.value) :
        ...
        mainbar.append("text").attr("class", "barpercent")
			...
			.text(function (d, i) { 
                const value = smartNumberFormatter ? smartNumberFormatter(d.percent*100) : 
                    (d.percent*100).toFixed(decimalPlaces);
                return "( " + value + "%) "; 
            })
			.attr("text-anchor", "end").style("fill", "grey")
            .attr("display", displayPercent);

        d3.select("#" + id).select(".part" + p).select(".subbars")
			.selectAll(".subbar").data(data.subBars[p]).enter()
			.append("rect").attr("class", "subbar")
			.attr("x", partWidth).attr("y", function (d) { return d.y })
			.attr("width", b).attr("height", function (d) {
			     return d.h
			})
    ...
    function drawEdges(data, id) {
        d3.select("#" + id).append("g").attr("class", "edges").attr("transform", "translate(" + (b + partWidth) + ",0)");
    ...
    function drawHeader(header, id) {
        ...
        [0, 1].forEach(function (d) {
            ...
            h.append("text").text(header[d]).attr("x", (c1[d] - 5))
				.attr("y", -5).style("fill", "grey").style("display", "none");
            ...
            h.append("line").attr("x1", c1[d] - 10).attr("y1", -2)
				...
				.style("stroke-width", "1").style("shape-rendering", "crispEdges").style("display", "none");
        });
    }
    ...
    bP.draw = function (data, svg) {
        data.forEach(function (biP, s) {
            svg.append("g")
				.attr("id", biP.id)
				.attr("transform", "translate(" + (1000 * s) + ",0)");
            ...
            [0, 1].forEach(function (p) {
                d3.select("#" + biP.id)
                        ...
                        .on("mouseover", function (d, i) {
                            return bP.selectSegment(data, p, d["key" + (p + 1)]); })
                        .on("mouseout", function (d, i) {
                            return bP.deSelectSegment(data, p, d["key"+ (p + 1)]); });
            });

        });
        bP.selectSegment(data, 0, 0);
    }
